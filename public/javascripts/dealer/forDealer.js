...
function drawTableInfo(tableInfo) {
	...
	//displaySmallBlindButton(tableInfo.button);
	displayBoard(board);
...
function displayDealerButton(seatId) {
	...
	$('#player'+(seatId+1)+'Small').addClass('on');
	$('.big').removeClass('on');
	$('#player'+(seatId+2)+'Big').addClass('on');
}
...
function setLayoutRound() {
	...
	var boardTop  = parseInt(0.6*displayAreaHeight); // 360:210 = displayAreaHeight: y
	var boardLeft = parseInt(0.65*displayAreaWidth); // 640:203 = displayAreaWidth: x
